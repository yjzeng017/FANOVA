...
ACOSSO <- function(X, y, order=2, wt.pow=1, cv='bic', w, lambda.0, gcv.pen=1.01, categorical="auto", linear=NULL, min.distinct=7){
...
## linear - vector containg the columns to be treated as linear 
...
if(!is.numeric(cv)){
  ans.cv <- venus.cv(X, y, order=order, cv=cv, wt.pow=wt.pow, w='L2', f.est='trad', seed=110, K.lim='data', nK=5, gcv.pen=gcv.pen, rel.K=5E-2, nlambda=5, lambda.lim=c(1E-10, 1E3), rel.lambda=5E-2, rel.theta=1E-2, maxit=2, init.cv='gcv', cat.pos=categorical, lin.pos=linear, min.distinct=min.distinct)

  lambda.0 <- ans.cv$lambda.0
  K <- ans.cv$K
  fit.acosso <- venus(X, y, order=order, K=ans.cv$K, gcv.pen=gcv.pen, maxit=5, rel.tol=1E-2, cv='gcv', lambda.0=ans.cv$lambda.0, w=ans.cv$w, seed=110, cat.pos=ans.cv$cat.pos, lin.pos=ans.cv$lin.pos, min.distinct=min.distinct)
}

else{  ## use a prespecified numerical value for K
  K <- cv
  if(missing(w)||missing(lambda.0)){
    ans.cv <- venus.cv(X, y, order=order, cv='gcv', wt.pow=wt.pow, w='L2', f.est='trad', seed=110, K.lim='data', nK=5, gcv.pen=gcv.pen, rel.K=5E-2, nlambda=5, lambda.lim=c(1E-10, 1E3), rel.lambda=5E-2, rel.theta=1E-2, maxit=2, init.cv='gcv', cat.pos=categorical, lin.pos=linear, min.distinct=min.distinct)
    w <- ans.cv$w
    lambda.0 <- ans.cv$lambda.0
  }
  fit.acosso <- venus(X, y, order=order, K=K, gcv.pen=gcv.pen, maxit=5, rel.tol=1E-2, cv='gcv', lambda.0=lambda.0, w=w, seed=110, cat.pos=ans.cv$cat.pos, lin.pos=ans.cv$lin.pos, min.distinct=min.distinct)
}

fit.acosso$order <- order
fit.acosso$K <- K
fit.acosso$lambda.0 <- lambda.0
fit.acosso$categorical <- categorical
fit.acosso$linear <- linear

end_time = Sys.time()
runtime = as.numeric(difftime(end_time, start_time, units = "secs"))
cat("The ACOSSO took", runtime, "seconds.\n")
return(fit.acosso)

}

ACOSSO.predict = function(fit, Xnew){
  # fit: output of ACOSSO
  # Xnew: new design
  X = fit$X
  y = fit$y
  p <- ncol(X)
  if(fit$order == 1) {P = p } else P = p + choose(p, 2)
  
  Gram.new = get.gram(X1=X, X2=Xnew, order = fit$order, cat.pos = fit$cat.pos, lin.pos = fit$lin.pos)
  Knew <- matrix(0, nrow(X)[1], nrow(Xnew)[1])
  for(j in 1:P)
    Knew = Knew + fit$theta[j]* fit$w[j]^2 *Gram.new[[j]]
  y_pred = (t(Knew)%*%fit$c.hat)[, 1] + fit$mu.hat 
  return(y_pred)
}


ACOSSO.selection = function(fit){
  p = ncol(fit$X)
  theta = fit$theta
  select.main = which(theta[1:p] > 0.0)
  select.int = NULL
  if(fit$order == 2 & length(which(theta[p+1:length(theta)] > 0.0)) > 0) {
    select.int = c()
    next.ind = p + 1
    for (i in index(1,p-1)) {
      for (j in index(i+1, p)) {
        if(theta[next.ind] > 0.0){
          select.int = c(select.int, c(i, j))
        }
        next.ind <- next.ind + 1
      }
    }
...
ACOSSO.predict.component = function(fit, Xnew){
  # fit: output of ACOSSO
  # Xnew: new design
  X = fit$X
  y = fit$y
  ...
  ncurves = matrix(0, nrow = nrow(Xnew)[1], ncol = P)
  Gram.new = get.gram(X1=X, X2=Xnew, order = fit$order, cat.pos = fit$cat.pos, lin.pos = fit$lin.pos)
  # Knew <- matrix(0, nrow(X)[1], nrow(Xnew)[1])
  for(j in 1:P) {
    Knew = fit$theta[j]* fit$w[j]^2 *Gram.new[[j]]
    ncurves[, j] = (t(Knew)%*%fit$c.hat)[, 1]
  }
  # y_pred = (t(Knew)%*%fit$c.hat)[, 1] 
  return(ncurves)
}
...
get.gram <- function(X1, X2, order, cat.pos, lin.pos){
 ...
 ## Calculates K(X1[i,j]
  ...
  for(i in cont.pos){
    ...
    x2 <- rep(X2[,i], each=n1)
    if(i %in% lin.pos){ans <- K.lin(x1,x2)}
    if(!(i %in% lin.pos)){ans <- K.sob(x1,x2)}
...
pen.spline <- function(X, y, order=1, lambda.0=1, theta, gcv.pen=1.01,
 ...
 ## Get Gram Matrix
  if(missing(Gram)){
   ## Get Gram Matrix
    Gram <- get.gram(X, X, order=order, cat.pos=numeric(0), lin.pos=numeric(0))
  }
...
venus <- function(X, y, K, order=1, gcv.pen=1.01,lambda.0, theta.0, rel.tol=1E-2,
                  ...
                  alpha=.05, nvar=ncol(X), nfit=20, cat.pos="auto", lin.pos=NULL,
                  min.distinct=7){
  ...
  if(is.null(lin.pos)){lin.pos=numeric(0)}
 ...
 ## Get Gram Matrix
  if(missing(Gram)){
    Gram <- get.gram(X, X, order=order, cat.pos=cat.pos, lin.pos=lin.pos)
  }
 ...
 ############### Use gcv ##################
  ...
  return(list(c.hat=c.hat, mu.hat=mu.hat, y.hat=y.hat, res=res, dfmod=df,
              ...
              cat.pos=cat.pos, lin.pos=lin.pos))
}
...
get.lambda.w <- function(X, y, order, lambda.lim, nlambda, w='L2', 
                         ...
                         nK, seed, cv, init.cv='gcv', cat.pos, lin.pos, min.distinct){

  if(f.est=='trad'){
    ans1 <- get.w(X=X, y=y, order=order, lambda.lim=lambda.lim, 
                  nlambda=nlambda, gcv.pen=gcv.pen, wt.pow=wt.pow, 
                  rel.tol=rel.tol, w=w, w.lim=w.lim, cat.pos=cat.pos, lin.pos = lin.pos)
    ans1$Gram <- NULL
    w <- ans1$w
    ...
    ans1 <- venus.cv(X, y, order=order, w=1, wt.pow=0, seed=seed, K.lim='data',
                     ...
                     cat.pos=cat.pos, lin.pos=lin.pos, min.distinct=min.distinct)
    ans1$Gram <- NULL
    fit.venus <- venus(X, y, order=order, K=ans1$K, gcv.pen=gcv.pen, maxit=5, 
                       rel.tol=1E-2, cv='gcv', lambda.0=ans1$lambda.0, 
                       w=ans1$w, cat.pos=cat.pos, lin.pos = lin.pos, min.distinct=min.distinct)
...
venus.cv <- function(X, y, order=1, w='L2', K.lim='data', nK=5, gcv.pen, 
             ...
             init.cv='gcv', cat.pos="auto", lin.pos=NULL, min.distinct=7){
 ...
 ## First get lambda.0 
  if(lambda.0=='est' || missing(Gram)){
    ans.lambda0 <- get.lambda.w(X=X, y=y, order=order, lambda.lim=lambda.lim, 
                   ...
                   nK=nK, seed=seed, cv=cv, init.cv=init.cv, cat.pos=cat.pos, lin.pos=lin.pos,
                   min.distinct=min.distinct)
    w <- ans.lambda0$w
...
#######################################
 ...
 ## Now conduct grid search on K
  K.min <- K.lim[1]
  K.max <- K.lim[2]
  KMIN <- log10(K.min+1.1)
  KMAX <- log10(K.max+1.1)
  log.K.min <- KMIN
  log.K.max <- KMAX
  ...
  return(list(K=K.best, lambda.0=lambda.0, theta=theta.best, Gram=Gram,
              pen=pen.best, w=w, cat.pos=cat.pos, lin.pos=lin.pos))
}
