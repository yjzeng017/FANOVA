...
splits = pickle.load(open(split_path + "parkinson_splits.pkl", "rb"))


df = pd.read_csv(data_path+'parkinson_updrs.data', sep=',',header=0)
...
exclude_cols = ['sex', 'motor_UPDRS', 'total_UPDRS']
...
X_scaled = np.array(df.drop(columns=['total_UPDRS', 'motor_UPDRS']))
...
def one_fit(j, method, omega):
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        if method == 'GPANOVA':
            ...
            model.fit(omega=omega, gamma=1e-3, verbose=True)
        else:
            model = SSANOVAModel(
                X=X_train,
                Y=Y_train.reshape(N_train, 1),
            ...
            model.fit(delta=0.001/np.log(N_train), omega=omega, max_iter=150, verbose=True)
        # evaluation
...
if __name__ == "__main__":
    for method in ['GPANOVA', 'SSANOVA']:
        f = open(root_path + f'real/parkinson_{method}_results.txt', 'w')
        f.write("========================================================\n")
        f.write(
            "parkinson's telemonitoring data: interaction model evaluated based on random splits for {0} repetitions ==============\n".format(
                rep))
        f.write("========================================================\n")
        for omega in omega_list:
            ...
            pickle.dump(results, open(result_path + "parkinson_{0}_w_{1}_{2}splits_results.pkl".format(method, omega, rep), 'wb'))
