...
# load boston data
df <- read.csv(paste(data_path, "/boston.data", sep=''), header = TRUE)
splits = pickle$load(builtins$open(paste(split_path, 'boston_splits.pkl', sep = ''), 'rb'))

# Drop some variables
df <- df %>% select(-all_of(c('town', 'tract', 'medv', 'chas','lon','lat')))
# Scale variables to [0, 1] except 'cmedv', the response, and 'chas', the category variable
# We define a helper function for Min-Max scaling
df <- df %>%
  mutate(across(-c(cmedv), function(x) {
    return((x - min(x)) / (max(x) - min(x)))
  }))

# Split into X and y
X.scaled = df %>% select(-c(cmedv))
y = df$cmedv
...
# open a .txt file
file_name = paste(root_path, 'real/boston_baselines_results.txt', sep = '')
writeLines("#######################################", file_name)
cat(" boston data: Interaction model with tenfold CV for ", npar/10, 'repetitions.', '\n', file = file_name, append = TRUE)
cat("####################################### \n", file = file_name, append = TRUE)
...
for(m in c('MARS', 'COSSO', 'ACOSSO','BSSANOVA')) {
  ...
  save(df.res, file = paste(result_path, 'boston_', m, '_10cv_results.Rdata', sep = ''))
  # sumamry
