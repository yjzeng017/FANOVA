...
splits = pickle.load(open(split_path + "jfk_splits.pkl", "rb"))


df = pd.read_csv(data_path+'jfk.data', sep=',',header=0)
feature_names = [
...
df['plane_age'] = 2024 - df['year']
df = df.dropna(axis=0, how='any')
X = np.array(df[feature_names], dtype=np.float64)
X_scaled = (X - X.min(axis=0))/ (X.max(axis=0) - X.min(axis=0))
Y = np.array(df['arr_delay'], dtype=np.float64) / 60
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        if method == 'GPANOVA':
            model = GPANOVAModel(
                ...
                linear_variables=[7, 8, 9, 10, 11, 12]
            )
            ...
            model.fit(omega=omega/(N_train+2), delta=.001/np.log(N_train), max_iter=100, verbose=True)
...
if __name__ == "__main__":
    for method in ['GPANOVA', 'SSANOVA']:
        f = open(root_path + f'real/jfk_{method}_results.txt', 'w')
        f.write("========================================================\n")
        f.write("JFK flight data: interaction model evaluated based on random splits for {0} repetitions\n".format(rep))
        f.write("========================================================\n")
        for omega in omega_list:
            ...
            pickle.dump(results, open(result_path + "jfk_{0}_w_{1}_{2}splits_results.pkl".format(method, omega, rep), 'wb'))
