...
# load pumadyn data
df <- read.csv(paste(data_path, "pumadyn.data", sep=''), sep = '', header = FALSE)
splits = pickle$load(builtins$open(paste(root_path, 'real/pumadyn/pumadyn_splits.pkl', sep = ''), 'rb'))
colnames(df) = c('V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'y')
# Scale variables to [0, 1]
continuous.features = c('V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8')
X.scaled = apply(df[continuous.features], 2, function(x) {
...
# open a .txt file
file_name = paste(root_path, 'real/pumadyn_baselines_results.txt', sep = '')
writeLines("#######################################", file_name)
cat(" pumadyn data: Interaction model with random splits for for ", npar, 'repetitions.', '\n', file = file_name, append = TRUE)
cat("####################################### \n", file = file_name, append = TRUE)

  
for(m in c('MARS', 'COSSO', 'ACOSSO')) {
  cat('\n','pumadyn data: method = ', m, '\n')
  ...
  save(df.res, file = paste(result_path, 'pumadyn_', m, '_', npar,'splits_results.Rdata', sep = ''))
  # sumamry
